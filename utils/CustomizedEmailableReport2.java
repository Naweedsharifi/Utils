...
public class CustomizedEmailableReport2 implements IReporter {
    ...
    private static final Logger L = Logger.getLogger(CustomizedEmailableReport2.class);
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, "Execution Report.html"))));
	...
	private void resultDetail(IResultMap tests) {
		...
		for (ITestResult result : list) {
			...
			if(hasParameters)
			{
				out.println("<h2 id=\"m" + methodIndex + "\">" + method.getMethodName()+Utils.toString(parameters[0]) + "</h2>");
			}
			else {
					...
					for (String line : msgs) {
						//if (!line.contains("<a")) {
							...
							for (int i = 0; i < straraay.length; i++) {
								...
								String sTestClassName = ans.getTestClass().getRealClass().getSimpleName();
								String sFileName = ans.getName()+i;
								WebDriver driver = WebDriverManager.gerDriverDetails(ans.getTestClass().getName());
								...
								if (straraay[i].equals("Fail")) {
									...
									out.println("<a href=" + imagePathInHost + "> Fail </a>");
								} else if (straraay[i].equals("Pass")) {
									//out.println("<td class=\"pass\">" + straraay[i] + "</td>");
									out.println("<a href=" + imagePathInHost + "> Pass </a>");
								} else {
						...
						if (line.contains("<a")) {
							...
							for (int i = 0; i < straraay.length; i++) {
								...
								String imagePathInHost = "file:results"+Seperator+ "screenshots" + Seperator
										...
										+ captureScreenshot(driver, sTestClassName, sFileName);
								Reporter.log("<a href=" + imagePathInHost + "> Fail </a>");*/
								if (straraay[i].contains("Fail")&& straraay[i].contains("<a") ) {
									//out.println("<td class=\"fail\">" + straraay[i] + "</td>");
									//out.println("<a href=" + imagePathInHost + "> Fail </a>");
									out.println(straraay[i] );
								} else if (straraay[i].contains("Pass")&& straraay[i].contains("<a")) {
									//out.println("<td class=\"pass\">" + straraay[i] + "</td>");
									//out.println("<a href=" + imagePathInHost + "> Pass </a>");
									out.println(straraay[i] );
								} else {
